...
public class AppiumAndroidDriver extends AppiumBaseDriver {
	...
	private void setAndroidAppCapabilities(DesiredCapabilities capabilities) throws Exception {
		...
		String strMobileAndroidApp = FilePaths
				.getResourcePath("/app/" + android_configuration.getProperty("appium.android.app"));
	 ...
	 * @throws Exception
	 */
	public void createSauceLabDriver(String deviceName,String platformName,String platformVersion) throws Exception {
		...
		try {
			...
			driver = new AndroidDriver(new URL(strAppiumServer), capabilities);
			Log.info("Starting remote Android driver for: " + capabilities.toString());
		} catch (Exception e) {
			Log.error("Can't start the webdriver on saucelab !!! : \n" + e.getMessage());
			throw (e);
		}
	}
