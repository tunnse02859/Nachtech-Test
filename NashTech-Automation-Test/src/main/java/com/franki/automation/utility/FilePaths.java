...
public class FilePaths {
	...
	public static void initReportFolder2() throws IOException {
		String awsReportRoot = System.getenv("DEVICEFARM_LOG_DIR");
		if(awsReportRoot != null) {
			REPORT_FOLDER = awsReportRoot + File.separator + "Reports";
			SCREENSHOT_FOLDER = REPORT_FOLDER + File.separator + "Screenshots";
			...
			REPORT_FOLDER = getRootProject() + File.separator + "Reports" + File.separator + "Latest Report";
			if(pathExist(REPORT_FOLDER)) {
				System.out.println("Renaming file to:");
				File oldReport = new File(REPORT_FOLDER);
				...
				System.out.println(renameOldReport.getAbsolutePath());
				System.out.println(oldReport.getAbsolutePath());
			}
			SCREENSHOT_FOLDER = REPORT_FOLDER + File.separator + "Screenshots";
		}
		if(pathExist(REPORT_FOLDER)) {
			System.out.println("folder still there");
		}
		createDirectory(REPORT_FOLDER);
		createDirectory(SCREENSHOT_FOLDER);
	}
	
	public static void initReportFilePath(String deviceName, String platformName, String platformVersion) throws IOException {
		String awsReportRoot = System.getenv("DEVICEFARM_LOG_DIR");
		if(awsReportRoot != null) {
			REPORT_FILE_PATH = REPORT_FOLDER + File.separator + "Report_" + System.getenv("DEVICEFARM_DEVICE_NAME") + ".html";
		}else {
			REPORT_FILE_PATH = REPORT_FOLDER + File.separator + "Report_" + deviceName + "_" + platformName + "_" + platformVersion + ".html";
		}
	}
